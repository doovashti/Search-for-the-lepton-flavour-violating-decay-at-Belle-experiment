...
t_sig->SetBranchAddress("bdtg_continuum", &sig_bdtgs);
...
c_bkg_charm->SetBranchAddress("bdtg_continuum", &bkg_bdtgs_charm);
...
c_bkg_uds->SetBranchAddress("bdtg_continuum", &bkg_bdtgs_uds);


///////////////////////////Creating the new file to store the PUNZI FOM//////////////////////////
TFile *f_out = new TFile("punzi_fom.root","RECREATE");
...
Float_t cut; // cut value ond the bdt score
...
for (float cuts =lowerrange; cuts < upperrange; cuts+=stepsize){
  ...
  for(int i =0; i<entries_bkg_charm; ++i){
    ...
    if (bkg_bdtgs_charm<cuts) continue;
    bkgmc++;
  } 
  ...
  for(int i =0; i<entries_bkg_uds; ++i){
    ...
    if (bkg_bdtgs_uds<cuts) continue;
    bkgmc++;
  }
    
  if((sigmc + bkgmc)==0) continue;
    
  eff_cut = sigmc/processedevent; //The efficiency for the BDT at each cut on the bdt scores
  expectedsig = eff_cut*sigtheory; //The expected # of signal events in BELLE for each efficiency of the BDT
...
TCanvas *c1 = new TCanvas("c1", "Punzi FOM vs BDTG Cut", 1024, 786);
...
std::cout << "The optimal cut for the BDTG score is: " << cut_max << std::endl;
